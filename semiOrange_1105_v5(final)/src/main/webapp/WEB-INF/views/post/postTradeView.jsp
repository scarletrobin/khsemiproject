		...
		<div class="postview-wrap">
			...
			<fieldset>
				...
				<table class="table table-bordered" id="postView" style="width:100%;">
					...
					<!-- 아래 tr부분 삭제해주세요 -->
					<tr >
						<th class="table-active">읽음 여부</th>
						<td colspan="2">${p.isRead } (테스트용이니까 삭제)</td>
					</tr>
					<tr >
						<th class="table-active">상품 번호</th>
						<c:choose>
							<c:when test="${empty p.tradeNo }">
								<td colspan="2">이게 떠야 정상 </td>
							</c:when>
							<c:otherwise>
								<td colspan="2">${p.tradeNo } </td>							
							</c:otherwise>
